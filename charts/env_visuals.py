...
Provides clean, responsive visualization modules completely driven by framework records dataframes.
"""
...
def render_waste_efficiency_hybrid_chart(df_long: pd.DataFrame, units_registry: dict):
    ...
    if prod_df.empty or waste_df.empty:
        ...
        return
    b_grp = prod_df.groupby("Month", as_index=False)["Value"].mean()
    l_grp = waste_df.groupby("Month", as_index=False)["Value"].mean()

    prod_unit = units_registry.get(prod_df["Metric"].iloc[0], "")
    waste_unit = units_registry.get(waste_df["Metric"].iloc[0], "")

    fig = go.Figure()
    fig.add_trace(go.Bar(x=b_grp["Month"], y=b_grp["Value"], name=f"Production Output ({prod_unit})", marker_color="#E2E8F0", opacity=0.8, yaxis="y1"))
    fig.add_trace(go.Scatter(x=l_grp["Month"], y=l_grp["Value"], name=f"Waste Intensity ({waste_unit})", mode="lines+markers", line=dict(color=PAL["primary"], width=3, shape="spline"), yaxis="y2"))
    fig.update_layout(
...
def render_waste_stream_stacked_chart(df_long: pd.DataFrame, units_registry: dict):
    ...
    haz_data = df_lower[df_lower["norm_metric"].isin(haz_keys)].groupby("Month", as_index=False)["Value"].sum()
    non_haz_data = df_lower[df_lower["norm_metric"].isin(non_haz_keys)].groupby("Month", as_index=False)["Value"].sum()
    merged = pd.merge(non_haz_data, haz_data, on="Month", how="outer", suffixes=("_NonHaz", "_Haz")).fillna(0)

    fig = go.Figure()
    fig.add_trace(go.Bar(x=merged["Month"], y=merged["Value_NonHaz"], name="Non-Hazardous Component (kg)", marker_color=PAL["primary"]))
    fig.add_trace(go.Bar(x=merged["Month"], y=merged["Value_Haz"], name="Hazardous Component (kg)", marker_color=PAL["primary-2"]))
...
def render_water_discharge_spline(df_long: pd.DataFrame, units_registry: dict):
    ...
    if water_df.empty:
        return
    grp = water_df.groupby("Month", as_index=False)["Value"].mean()
    unit = units_registry.get(water_df["Metric"].iloc[0], "")
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=grp["Month"], y=grp["Value"], mode="lines+markers", fill="tozeroy", line=dict(color=PAL["success"], width=2.5, shape="spline"), fillcolor="rgba(16, 185, 129, 0.05)"))
...
def render_dynamic_hybrid_overlay(df_filtered: pd.DataFrame, title: str, bar_metric: str, line_metric: str, units_registry: dict):
    ...
    if bar_df.empty or line_df.empty:
        ...
        return
    b_grp = bar_df.groupby("Month", as_index=False)["Value"].mean()
    l_grp = line_df.groupby("Month", as_index=False)["Value"].mean()

    b_unit = units_registry.get(bar_df["Metric"].iloc[0], "")
    l_unit = units_registry.get(line_df["Metric"].iloc[0], "")

    fig = go.Figure()
    fig.add_trace(go.Bar(x=b_grp["Month"], y=b_grp["Value"], name=f"{bar_metric} ({b_unit})", marker_color="#E2E8F0", opacity=0.85, yaxis="y1"))
    fig.add_trace(go.Scatter(x=l_grp["Month"], y=l_grp["Value"], name=f"{line_metric} ({l_unit})", mode="lines+markers", line=dict(color=PAL["primary"], width=3, shape="spline"), yaxis="y2"))
    fig.update_layout(
...
def render_single_trajectory_area(df_filtered: pd.DataFrame, target_metric_substring: str, title: str, color_hex: str, units_registry: dict):
    ...
    if sub_df.empty:
        return
    grp = sub_df.groupby("Month", as_index=False)["Value"].mean()
    unit = units_registry.get(sub_df["Metric"].iloc[0], "")
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=grp["Month"], y=grp["Value"], mode="lines+markers", fill="tozeroy", line=dict(color=color_hex, width=2.5, shape="spline"), fillcolor=f"{color_hex}12"))
