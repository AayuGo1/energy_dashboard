"""
Data Pipeline Hardening Subsystem — Robust File Ingestion & Persistence Framework.
Handles remote requests to GitHub and serves local snapshots during connectivity drops.
"""
import os
import json
import logging
...
def fetch_workbook_hardened(url: str, cache_key: str, _fetch_fn):
    ...
    try:
        ...
        logger.info(f"Workbook successfully streamed live from production endpoint: {url}")
        return content, {
        ...
        if fallback is not None:
            ...
            meta = dict(meta)
            meta["warning"] = f"Live source unreachable ({e}). Displaying local storage snapshot."
            meta["source"] = "fallback"
            logger.warning(meta["warning"])
            return content, meta
        logger.error("No fallback snapshot found on disk storage systems.")
        raise
...
def validate_schema(file_bytes: bytes) -> list:
    ...
    for required in config.REQUIRED_SHEETS:
        if required not in sheet_names:
            issues.append(f"Required base database table layer sheet missing from workbook: '{required}'")
    wb.close()
    return issues
